...
def pack_trucks_from_csv(csv_file_path):
    ...
    for _, row in df.iterrows():
        for _ in range(row["total_orders"]):
            expanded_rows.append({
                "customer_name": row["customer_name"],
                "priority": row["priority"],
                "weight": BOX_WEIGHT  # Or your chosen default
            })

    expanded_df = pd.DataFrame(expanded_rows)

    # Step 3: Add box_number per customer
    expanded_df["box_number"] = expanded_df.groupby("customer_name").cumcount() + 1

    # Step 4: Sort by priority
    expanded_df = expanded_df.sort_values(by="priority", ascending=True)

    boxes = expanded_df.to_dict("records")
    ...
    for box in boxes:
        ...
        for truck in truck_fleet:
            ...
            if len(truck["boxes"]) == maxCapacity:
                continue
            used_volume = len(truck["boxes"]) * BOX_VOLUME
            if (used_volume + BOX_VOLUME <= truck["volume"]) and (
                truck["weight"] + box["weight"] <= truck["max_weight"]
            ):
